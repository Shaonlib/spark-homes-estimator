...
Vanilla JS/HTML/CSS. CDN libraries: [JSZip](https://cdnjs.com/libraries/jszip) (ZIP export), [SheetJS/xlsx](https://cdnjs.com/libraries/xlsx) (Excel export), [Tesseract.js](https://cdnjs.com/libraries/tesseract.js) (lazy-loaded, serial-number OCR only). Google Fonts: Barlow Condensed, Inter, IBM Plex Mono.

## Running locally

No install needed. Two options:

1. Double-click `index.html` — most features work directly from the filesystem, though the service worker (offline caching) requires a server context in some browsers.
2. Serve the folder so the service worker registers cleanly:
   ```bash
   cd spark-homes-estimator
